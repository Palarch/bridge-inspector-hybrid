...
# --- 1. CONFIG & CSS SETUP ---
st.set_page_config(page_title="Hybrid Bridge Inspector (Final)", layout="wide")
...
<style>
    ...
    /* Urgency Badges (Standardized colors) */
    .urgency-1 { background-color: #dc3545; color: white; padding: 6px 15px; border-radius: 20px; font-weight: bold; font-size: 16px; } /* Red */
    .urgency-2 { background-color: #fd7e14; color: white; padding: 6px 15px; border-radius: 20px; font-weight: bold; font-size: 16px; } /* Orange */
    .urgency-3 { background-color: #28a745; color: white; padding: 6px 15px; border-radius: 20px; font-weight: bold; font-size: 16px; } /* Green */
...
# --- 2. HYBRID LOGIC ENGINE (DOH -> Mapping -> Pellegrini) ---
...
# --- 3. HIGH-DENSITY STRUCTURE GENERATOR (Volumetric) ---
def generate_complex_structure(defect_type, component_name):
    points_list = []
    
    # Function to create dense volume blocks
    def add_dense_block(x_lim, y_lim, z_lim, density=2500):
        vol = (x_lim[1]-x_lim[0]) * (y_lim[1]-y_lim[0]) * (z_lim[1]-z_lim[0])
        n_points = int(density * vol * 5)
        if n_points < 300: n_points = 300 # Minimum points
        
        # Random Uniform Distribution for solid look
        ...
        # Surface points (for sharper edges)
        xe = np.linspace(x_lim[0], x_lim[1], 30)
        ye = np.linspace(y_lim[0], y_lim[1], 30)
    ...
    # === BUILD BRIDGE ===
    # 1. Deck
    add_dense_block([0, L], [0, W], [z_deck_bot, 0], density=3000)
    
    # 2. Girders (3 Lines)
    gy_list = [2.0, 4.0, 6.0]
    for y in gy_list:
        add_dense_block([0, L], [y-0.3, y+0.3], [z_girder_bot, z_deck_bot], density=2500)
        
    # 3. Diaphragms (Cross beams)
    for x in [0.5, L/2, L-0.5]:
        add_dense_block([x-0.15, x+0.15], [gy_list[0], gy_list[1]], [z_girder_bot+0.3, z_deck_bot-0.1])
        add_dense_block([x-0.15, x+0.15], [gy_list[1], gy_list[2]], [z_girder_bot+0.3, z_deck_bot-0.1])

    # 4. Substructure (Cap & Pier)
    for sx in [2.0, 10.0]:
        add_dense_block([sx-0.6, sx+0.6], [0.5, W-0.5], [z_cap_bot, -1.5], density=4000) # Cap
        for py in [2.5, 5.5]:
            add_dense_block([sx-0.4, sx+0.4], [py-0.4, py+0.4], [z_pier_bot, z_cap_bot], density=3000) # Pier

    full = np.concatenate(points_list, axis=0)
    X, Y, Z = full[:,0], full[:,1], full[:,2]
    
    # === SIMULATE DEFECTS ===
    Z += np.random.normal(0, 0.005, size=Z.shape) # Texture Noise
    ai_depth = 0.0
    ...
    if uploaded_file is not None:
        try:
            ...
            if {'x','y','z'}.issubset(df.columns):
                if len(df) > 50000: df = df.sample(50000)
...
# Main Page
st.title("🌉 Hybrid Bridge Inspector Final")
st.caption("Feature: High-Density 3D + Cross-Section + DOH/Pellegrini Standards")
...
with col_viz:
    ...
    if len(X) > 0:
        ...
        # Filter Logic (Thinner slice for better visibility)
        mask = np.abs(X - slice_pos) < 0.2
        
        # 2D Plot
        fig_sec = go.Figure(go.Scatter(
            x=Y[mask], y=Z[mask], mode='markers', 
            marker=dict(size=4, color=Z[mask], colorscale='Jet_r', opacity=0.8)
        ))
        ...
        fig_3d = go.Figure(data=[go.Scatter3d(
            ...
            marker=dict(size=1.5, color=Z, colorscale='Jet_r', opacity=0.8) # Smaller points for density
        )])
        # Plane
