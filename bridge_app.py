...
# --- 1. CONFIG & CSS ---
st.set_page_config(page_title="Hybrid Bridge Inspector v6.7", layout="wide")
...
# --- 4. STRUCTURE GENERATOR (With Severity Map) ---
def generate_complex_structure(defect_type, component_name):
    ...
    for sx in [2.0, 10.0]:
        ...
        for by in [2.0, 4.0, 6.0]: add_dense_block([sx-0.2, sx+0.2], [by-0.2, by+0.2], [-1.5, -1.4], density=800) # Bearing
        for py in [2.5, 5.5]: 
    ...
    if defect_type != "No Defect":
        ...
        if np.any(mask):
            ...
            # --- SEVERITY MAPPING ---
            # Assign High Value to Defect Area
            S[mask] = 1.0 # Critical (Red)
...
def generate_mock_batch():
    ...
    for _ in range(5):
        ...
        batch.append({
            ...
            "group": group, "comp": comp, "comp_th": BRIDGE_SCHEMA[group][comp]["name_th"],
    ...
    if uploaded_file is not None:
        try:
            ...
            if {'x','y','z'}.issubset(df.columns):
                ...
                S = np.zeros_like(Z) # No auto-defect detection for real files yet
...
st.title("🌉 Hybrid Bridge Inspector v6.7 (Visualization Switcher)")
st.caption("Standards: DOH Detection ➔ Pellegrini Management | Feature: Elevation vs. Severity Mode")
...
if st.session_state.idx >= len(st.session_state.mock_data):
    ...
    with col1:
        st.download_button("📥 Download Report", df_res.to_csv(index=False).encode('utf-8-sig'), f"Report_{datetime.now().strftime('%H%M')}.csv", "text/csv", type="primary")
    with col2:
...
with col_viz:
    ...
    with c1:
        st.caption(f"Defect: {item['type']}")
    with c2:
        # Toggle Button
        view_mode = st.radio("Display Mode:", ["Elevation (Height)", "Severity (Damage)"], horizontal=True)

    if len(X) > 0:
        # Determine Color Logic
        if view_mode == "Elevation (Height)":
            color_data = Z
            colorscale = 'Jet_r'
            c_title = "Elevation (m)"
        else: # Severity Mode
            color_data = S
            colorscale = 'RdYlGn_r' # Green (0) -> Red (1)
            c_title = "Severity Index"

        # 3D Plot
        fig_3d = go.Figure(data=[go.Scatter3d(
            x=X, y=Y, z=Z, 
            mode='markers', 
            marker=dict(size=3, color=color_data, colorscale=colorscale, opacity=0.8, showscale=True, colorbar=dict(title=c_title, thickness=15, x=1.0))
        )])
        
        # Add Plane only if showing Elevation (to not confuse in heatmap)
        ...
        # 2D Plot (Always Elevation for Analysis)
...
with col_data:
    ...
    <div style="border:1px solid #ddd; padding:10px; border-radius:5px;">
        ...
        Measured: <b>{ai_depth:.4f} m</b><br>Rating: <b>{doh}/5</b>
    </div>
    ...
    with st.expander("📘 Standards Reference"):
        t1, t2 = st.tabs(["🇹🇭 DOH", "🇪🇺 Pellegrini"])
        with t1: st.write("Rating 5 (Good) -> 1 (Critical)")
        with t2: st.write("Urgency: >6.0 Immediate, 3-6 Short-term, <3 Monitor")
